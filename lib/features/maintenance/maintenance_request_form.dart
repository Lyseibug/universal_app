...
/// The actual "raise a maintenance request" form -- machine auto-fills from
/// the caller's active workstation (or lets them pick from their assigned
/// ones if none is selected), issue type is filtered to that machine's
/// station type. Shared between the dedicated maintenance_request screen
/// (raise_maintenance_request_screen.dart) and the global Quick Support
/// drawer (widgets/pdt_scaffold.dart), so both stay in sync with one
/// implementation instead of drifting apart.
class MaintenanceRequestForm extends ConsumerStatefulWidget {
  /// Called after a successful submit -- e.g. to close the drawer it's
  /// embedded in. The standalone screen leaves this null and just resets
  /// the form in place instead.
  final VoidCallback? onSubmitted;

  const MaintenanceRequestForm({this.onSubmitted, super.key});
