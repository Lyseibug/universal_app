...
class _ManualTransferScreenState extends ConsumerState<ManualTransferScreen> {
  ...
  void _onBarcodeScanned(String barcode) {
    if (_fromFocus.hasFocus) {
      _fromLotCtrl.text = barcode;
      _toFocus.requestFocus();
    } else if (_toFocus.hasFocus) {
      _toLotCtrl.text = barcode;
      _itemFocus.requestFocus();
    } else if (_itemFocus.hasFocus) {
      _itemCtrl.text = barcode;
      _batchFocus.requestFocus();
    } else if (_batchFocus.hasFocus) {
      _batchCtrl.text = barcode;
      _qtyFocus.requestFocus();
    } else {
      // Fallback
      if (_fromLotCtrl.text.isEmpty) {
        _fromLotCtrl.text = barcode;
      } else if (_toLotCtrl.text.isEmpty) {
        _toLotCtrl.text = barcode;
      } else if (_itemCtrl.text.isEmpty) {
        _itemCtrl.text = barcode;
      } else if (_batchCtrl.text.isEmpty) {
        _batchCtrl.text = barcode;
      }
    }
    setState(() {});
