...
| 19 | Workstation `custom_pdt_screen_key` (auto-route after login) | Workstation list | ⚠️ set on `L1B1` only (test value `line2_building`), added 2026-07-10 — set on every real curing pot/building/processing Workstation to `line2_curing`/`line2_building`/`line2_processing` for auto-routing to work; blank = worker still lands on the tile menu, harmless — see §6.1a |
...
### 6.1a Real Job Card time tracking, parallel jobs, workstation auto-routing

**Redesigned 2026-07-10.** Previously, "elapsed time" on a Job Card was purely client-side (`DateTime.now()` at scan time) — nothing was ever persisted, and `complete_step` always recorded a fabricated zero-duration `time_logs` row. Real start/end time is now tracked on **Job Card itself** (core ERPNext `time_logs` child table, `actual_start_date`/`actual_end_date` auto-computed from it).

- **A worker can now have several Job Cards genuinely open in parallel** — one per workstation (each physical pot/station is its own `Workstation` record; a worker "runs several in parallel" by switching which workstation-scoped screen they're at, not by juggling multiple jobs on one screen). This required deliberately **never setting `employee`** on `Job Card Time Log` rows — that field triggers ERPNext's own built-in one-open-time-log-per-employee guard (`OverlapError`), which would otherwise block exactly this. Operator identity is tracked instead via a new `Job Card.custom_operator` field (Link → Employee), set once when a job starts.
- **Scanning a flowchart barcode both starts and resumes** a job: the first scan opens a real time log; re-scanning the same barcode later (after navigating away, or after killing/relaunching the app) is a safe idempotent read that shows the true accumulated elapsed time — it never resets to zero and never duplicates the log.
- **Active Jobs** (`line2_active_jobs`) is now a real personal "what's still running for me" hub: lists every Job Card you have open across all your workstations with a live, accurate elapsed timer. **Tap a job to jump straight back into its station screen**, pre-loaded (no re-scan needed) — long-press for the old read-only detail sheet.
- **Workstation auto-routing**: after selecting a workstation at login (or on app relaunch), the app opens that workstation's PDT screen directly instead of the tile menu — driven by the new `Workstation.custom_pdt_screen_key` field (setup checklist #19). Blank = unchanged behavior (lands on the tile menu).

**Verify:** scan a flowchart at one workstation, switch to a different workstation (e.g. via Active Jobs or the tile menu) and scan a second, different flowchart there — both should show live, independently-ticking timers in Active Jobs, with no `OverlapError`. Complete one — its time log should show a real non-zero duration in Desk (Job Card → Time Logs table). Log out and back in on a workstation with `custom_pdt_screen_key` set — the app should land directly on that screen.
